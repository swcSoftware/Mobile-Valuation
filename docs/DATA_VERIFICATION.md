...
| Diluted share count | `WeightedAverageNumberOfDilutedSharesOutstanding`, **scale-checked against net income ÷ diluted EPS** and rescaled when a filer tags millions as units (MCD) | `sec` (note records the correction) | Filer tagging errors; correction is applied only on a clean power of 1000 within 20% of the reference | The note on the value names the factor and the reference used |
